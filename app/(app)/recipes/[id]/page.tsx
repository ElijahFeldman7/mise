...
import { IngredientList, PortionsProvider, ServingsStepper } from "./Portions";
...
export default async function RecipePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const meta = [formatMinutes(recipe.total_minutes)].filter(Boolean);
  ...
  return (
    <>
      ...
      <PortionsProvider
        ...
        cookingFor={session.household.cooks_for ?? recipe.servings ?? 2}
      >
        <div className="flex flex-col gap-[18px] px-5 pt-[18px]">
          <div>
            ...
            <div className="mt-[9px] flex flex-wrap items-center gap-2 text-[12.5px] text-ink-soft">
              ...
              {meta.length ? <span className="text-ink-ghost">·</span> : null}
              <ServingsStepper />
              {recipe.oven_temp_f ? (
                <>
          ...
          {recipe.source_url ? (
            ...
            >
              from {recipe.import_domain ?? sourceName(recipe.source_url)} · view original
            </a>
          ) : null}
